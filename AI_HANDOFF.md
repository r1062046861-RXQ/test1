...
- 玩家选择体质后开始一局游戏。
- 地图节点驱动流程：普通战斗、精英、Boss、事件、商店、休息、宝箱、奖励。
...
| SVG | 2 个，5,727 bytes，约 0.01 MiB |
...
| `constitutions` | 3 | 925,244 bytes，约 0.88 MiB |
...
- `Constitution = 'balanced' | 'yin_deficiency' | 'qi_deficiency'`
...
| `reward` | `RewardView` | 战斗奖励 |
| `chest` | `ChestView` | 宝箱 |
| `rest` | `RestView` | 休息 |
| `shop` | `ShopView` | 商店 |
| `event` | `EventView` | 事件 |
| `game_over` | `App.tsx` 内联界面 | 失败结算 |
| `victory` | `App.tsx` 内联界面 | 胜利结算 |
...
体质由 `shared/baseTypes.ts` 的 `Constitution` 限定，局内初始化主要在 `game/src/store/gameStore.ts`。
...
注意：`startCombat(nodeId)` 在 store 中实际承担“进入地图节点”的分发职责，节点可能不是战斗，也可能进入商店、休息、事件或宝箱。
...
- 根目录素材文件夹大多是原始素材或导入来源，不应按“未跟踪”直接删除。
