...
## 第五轮进攻密度与手牌经济记录（2026-05-26）

- 每回合首张攻击牌消耗 -1，最低为 0；费用增加、脾虚湿盛和 Boss 土相仍会正常抬高攻击牌费用。
...
| 技能 | 44 | 44 (40.7%) | 1.0 | 玩家药方:6 / 玩家药材:38 | 普通:11 / 稀有:18 / 精良:15 |
...
列为所有出现在起始牌组里的卡牌，单元格为该体质初始拥有数量。第五轮已调整部分起始牌组；上方“起始牌组统计”和 `shared/data/cards.ts` 的 `STARTING_DECKS` 是当前真值。本矩阵仅用于历史横向扫视，后续若继续大规模换牌建议全量再生成后再作为真值使用。
...
| apply_weak | 1 | 玩家药材:1 | 攻击:1 | 1:1 | 2-2 / 均2 | 4-4 / 均4 | 点穴：合谷 |
| attack_pierce_all | 1 | 玩家药材:1 | 攻击:1 | 1:1 | 3-3 / 均3 | - | 细辛通窍 |
...
| cleanse_draw | 1 | 玩家药材:1 | 技能:1 | 1:1 | 2-2 / 均2 | 1-1 / 均1 | 赤芍凉血 |
...
| sleep_debuff | 1 | 玩家药材:1 | 攻击:1 | 1:1 | 4-4 / 均4 | - | 酸枣仁安眠 |
...
| baishao | 白芍柔肝 | 玩家药材 | 攻击 | 普通 | 1 | 单体敌人 |  | debuff_weak_draw | 1 | 3 |  | 造成3点伤害，使一个敌人虚弱(造成的伤害减少25%)，抽1张牌。 |
...
| equipment_ziwuliuzhu | 子午流注 | 装备被动 | 能力 | 精良 | 0 | 自身 |  | equipment_ziwuliuzhu |  |  | 是 | 装备牌：非首回合的玩家回合开始时，若手牌少于5张则额外抽1张牌，最多1件生效。 |
...
| equipment_ziwuliuzhu | 子午流注 | equipment_ziwuliuzhu | 精良 | 装备牌：非首回合的玩家回合开始时，若手牌少于5张则额外抽1张牌，最多1件生效。 |
