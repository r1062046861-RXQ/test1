...
- 当前版本：`1.0.0`
- 游戏首页显示版本号，来源为 `game/package.json` 的 `version` 字段。
- 以后更新 README、AI_HANDOFF、BALANCE_GUIDE、设计文档或数据表说明时，必须同时写明对应版本号；若修改会影响玩家可见内容或规则，先更新 `game/package.json` 版本号，再同步文档。
...
| 音频目录当前约35.6MiB；运行时资源当前工作区约198MiB，图片 manifest 约161MiB | ✅ |
| 首页/主菜单动图使用原始 GIF（首页≈46MB, 主菜单≈37MB），清晰度优先于体积 | ✅ |
...
| `1.0.0` 战斗效果面板拆为增益效果/负面效果/装备三栏，装备页展示真实装备卡图、数量与描述 | ✅ |
| `1.0.0` 战斗效果与装备条目可点击查看完整说明；体质被动收益和代价已拆分到增益/负面分栏 | ✅ |
| `1.0.0` 战斗手牌字体随卡牌尺寸放大，长悬浮 3 秒显示 1.5 倍卡牌预览 | ✅ |
| `1.0.0` 平衡更新：回血牌整体下调、平和/阴虚/气虚/阳虚/痰湿/气郁/特禀起始牌组补到 7 张攻击牌，普通敌难度上调、Boss 血量回落、药方牌增加配伍特色、普通敌招式增加状态联动 | ✅ |
| `1.0.0` 首页公告入口已展示更新日志，战斗平衡部分逐卡列出旧效果与新效果；公告红点首次点开后消失 | ✅ |
...
| `1.0.0` 手牌总览入口：地图页为“手牌一览”，药房和战斗奖励页为“查看现有手牌”，均含装备牌显示；总览内每张牌可继续点击查看详细属性 | ✅ |
...
当前验证（2026-06-02）：`npm test -- --run` 为 6 个测试文件、90 个测试用例通过。

`npm run build` 当前通过。主 JS chunk 约 497 kB，构建产物发布于 `game/dist/`。
