  ...
  return (
    <div className={cn('combat-parchment-panel flex min-h-0 flex-col overflow-hidden px-3 py-3', className)}>
      <div className="mb-2 flex items-center gap-2 border-b border-amber-900/10 pb-2">
        ...
        <div className="text-[12px] uppercase tracking-[0.24em] text-stone-600">战斗记录</div>
      </div>
      <div className={cn('ornate-scroll min-h-0 flex-1 overflow-y-auto pr-1', compact ? 'text-[12px] leading-5' : 'text-sm leading-6')}>
        <div className="flex flex-col gap-1.5">
          {combatLog.map((log, index) => (
            <div
              key={`${index}-${log}`}
              className="combat-parchment-inset px-2.5 py-1.5 text-stone-800 transition hover:bg-white/65"
            >
              {log}
            </div>
          ))}
